...
%% Exported Functions
%%
-export([do_get/2, do_post/2, test/0]).
...
%% 	end,
	...
	Req:ok({"text/plain; charset=utf-8", [{"server", "Mochiweb-Test"}], format(proplists:lookup("i", Data), Msg)}).
...
format(I, Msg) ->
	lists:flatten(io_lib:format("io.j[~p]('~s');", [I, Msg])).
